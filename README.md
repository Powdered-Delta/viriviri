...
- [x] 沉浸式推荐 panel 的系统 IME 搜索输入与 Bilibili WBI 视频搜索结果。
- [x] 推荐项选择、进入观看页、返回推荐页。
...
- [ ] 尚无视频播放控制按钮，例如播放/暂停、进度、音量和倍速。
- [ ] 需要在 Quest 上人工确认不同视频源比例下的 2D `TextureView` 黑边与居中显示。
...
- [ ] 推荐列表缺少不透明或半透明背景，文字可读性不足。
- [ ] 观看页缺少视频详情信息。
- [ ] 推荐条目缺少封面预览图和双行标题/副信息布局。
- [ ] 推荐列表及后续操作 panel 应与播放控制条一致，支持自动淡出与按交互唤回。
- [ ] 增加摇杆控制播放进度和音量。
- [ ] 研究巨幕与曲面屏幕模式。
